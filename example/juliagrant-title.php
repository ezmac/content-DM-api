...
// Delete all files of the form p16631coll2_title_msstatecv* before re-creating them again. 
print "CLEANING up old files..... <br> \n";
foreach (glob("p16631coll2_title_msstatecv*") as $filename) {
...
$full_URL_name = "cdm16631.contentdm.oclc.org/ui/custom/default/collection/coll_p16631coll2/resources/custompages/browse/";
...
//one collection //
//*******//
$curl_url =  "https://" . $server_name .  "/dmwebservices/index.php?q=dmGetCollectionParameters/p16631coll2/json";
...
foreach ($alphanumeric as $letter) {
  ...
  #$tempfile = "C:\\xampp\htdocs\p16631coll2_title_msstatecv_temp_" . "$letter";
 // this is for cdm hosted // $tempfile = "/cdm/sites/16631/Website/public_html/ui/custom/default/collection/coll_p16631coll2/resources/custompages/browse/" . "p16631coll2_title_msstatecv_temp_" . "$letter";
 $tempfile = "/var/www/cdm/scriptoutput/" . "p16631coll2_title_msstatecv_temp_" . "$letter";
...
$curl_url = "https://" . $server_name . "/dmwebservices/index.php?q=dmGetCollectionFieldVocabulary/p16631coll2/title/0/0/json";
...
foreach ($alphanumeric as $letter) {
    ...
    $tempfile = "/var/www/cdm/scriptoutput/" . "p16631coll2_title_msstatecv_temp_" . "$letter";
  $tempfile_out = "/var/www/cdm/scriptoutput/" . "p16631coll2_title_msstatecv_temp_" . "$letter" . "_sorted";
...
foreach ($alphanumeric as $letter) {
    ...
    $tempfile = "/var/www/cdm/scriptoutput/"  . "p16631coll2_title_msstatecv_temp_" . "$letter" . "_sorted";
  $tempfile_out = "/var/www/cdm/scriptoutput/"  . "p16631coll2_title_msstatecv_temp_" . "$letter" . "_dedupped";
  $handle_out = fopen("$tempfile_out", "a");
       $contents = file($tempfile);
...
print "STARTING TO POPULATE files with clickable links.... \n";
 foreach ($alphanumeric as $letter) {
    ...
    $tempfile = "/var/www/cdm/scriptoutput/"  . "p16631coll2_title_msstatecv_temp_" . "$letter" . "_dedupped";
  $tempfile_out = "/var/www/cdm/scriptoutput/"  . "p16631coll2_title_msstatecv_" . "$letter" . ".html";
  $handle_out = fopen("$tempfile_out", "a");
...
///////////####		  Limit collection here - change /all to the collection name///////////////////////
		 ...
		 $link_tmp = "<a href=\"http://cdm16631.contentdm.oclc.org/cdm/search/collection/p16631coll2/searchterm/$original/field/title/mode/exact\">$original</a>";
...
#exit;
make_nav:
// make the p16631coll2_title_msstatecv_navbar.html file 
print "STARTING TO MAKE NAVBAR...<br>\n";

$tempfile_out = "/var/www/cdm/scriptoutput/p16631coll2_title_msstatecv_navbar" . ".html";
$handle_out = fopen("$tempfile_out", "a");
foreach ($alphanumeric as $letter) {

   $tempfile_in = "/var/www/cdm/scriptoutput/p16631coll2_title_msstatecv_" . "$letter" . ".html";

  $size = filesize($tempfile_in);
  if ($size  > 0 ) {    // there are subjects in a file, some files don't have subjects!
     print "good--file p16631coll2_title_msstatecv_$letter.html has data <br>";
    if ( $letter === "0" ) {	 
...
$linenavbar = "<a href=\"http://blogs.library.msstate.edu/cdm/scriptoutput/p16631coll2_title_msstatecv_" . "$letter" . ".html\">$letter</a>\n";  // For non-msstate interface (generic)
...
foreach (glob("p16631coll2_title_msstatecv_*sorted") as $filename) {
      print "removing $filename <br>";
      unlink($filename) or print("could not unlink $filename ....does not exist <br>");
}  
foreach (glob("p16631coll2_title_msstatecv_*dedupped") as $filename) {
	  print "removing $filename <br>";
      unlink($filename) or print("could not unlink $filename ....does not exist <br>");
}
foreach (glob("p16631coll2_title_msstatecv*temp*") as $filename) {
      print "removing $filename <br>";
      unlink($filename) or print("could not unlink $filename ....does not exist <br>");
}
