...
// Delete all files of the form bow_test_time_msstatecv* before re-creating them again. 
print "CLEANING up old files..... <br> \n";
foreach (glob("bow_test_time_msstatecv*") as $filename) {
...
$full_URL_name = "cdm16631.contentdm.oclc.org/ui/custom/default/collection/coll_bow_test/resources/custompages/browse/";
...
//one collection //
//*******//
$curl_url =  "https://" . $server_name .  "/dmwebservices/index.php?q=dmGetCollectionParameters/bow_test/json";
...
foreach ($alphanumeric as $letter) {
  ...
  #$tempfile = "C:\\xampp\htdocs\bow_test_time_msstatecv_temp_" . "$letter";
 // this is for cdm hosted // $tempfile = "/cdm/sites/16631/Website/public_html/ui/custom/default/collection/coll_uac/resources/custompages/browsesubseries/" . "bow_test_time_msstatecv_temp_" . "$letter";
 $tempfile = "/var/www/cdm/scriptoutput/" . "bow_test_time_msstatecv_temp_" . "$letter";
 ...
 if (file_exists($tempfile)){
  ...
  for($i = 0; $i < $counter; ++$i) {
		...
		$ch = curl_init(); 
//		$curl_url = "http://" . $server_name . ":81/dmwebservices/index.php?q=dmGetCollectionFieldVocabulary/" . $alias . "/time/0/0/json";
...
$curl_url = "https://" . $server_name . "/dmwebservices/index.php?q=dmGetCollectionFieldVocabulary/bow_test/time/0/0/json";
...
foreach ($alphanumeric as $letter) {
    ...
    $tempfile = "/var/www/cdm/scriptoutput/" . "bow_test_time_msstatecv_temp_" . "$letter";
  $tempfile_out = "/var/www/cdm/scriptoutput/" . "bow_test_time_msstatecv_temp_" . "$letter" . "_sorted";
...
foreach ($alphanumeric as $letter) {
    ...
    $tempfile = "/var/www/cdm/scriptoutput/"  . "bow_test_time_msstatecv_temp_" . "$letter" . "_sorted";
  $tempfile_out = "/var/www/cdm/scriptoutput/"  . "bow_test_time_msstatecv_temp_" . "$letter" . "_dedupped";
  $handle_out = fopen("$tempfile_out", "a");
       $contents = file($tempfile);
...
print "STARTING TO POPULATE files with clickable links.... \n";
 foreach ($alphanumeric as $letter) {
    ...
    $tempfile = "/var/www/cdm/scriptoutput/"  . "bow_test_time_msstatecv_temp_" . "$letter" . "_dedupped";
  $tempfile_out = "/var/www/cdm/scriptoutput/"  . "bow_test_time_msstatecv_" . "$letter" . ".html";
  $handle_out = fopen("$tempfile_out", "a");
...
///////////####		  Limit collection here - change /all to the collection name///////////////////////
		 ...
		 $link_tmp = "<a href=\"http://cdm16631.contentdm.oclc.org/cdm/search/collection/bow_test/searchterm/$original/field/time/mode/exact\">$original</a>";
...
#exit;
make_nav:
// make the bow_test_time_msstatecv_navbar.html file 
print "STARTING TO MAKE NAVBAR...<br>\n";

$tempfile_out = "/var/www/cdm/scriptoutput/bow_test_time_msstatecv_navbar" . ".html";
$handle_out = fopen("$tempfile_out", "a");
foreach ($alphanumeric as $letter) {

   $tempfile_in = "/var/www/cdm/scriptoutput/bow_test_time_msstatecv_" . "$letter" . ".html";

  $size = filesize($tempfile_in);
  if ($size  > 0 ) {    // there are subjects in a file, some files don't have subjects!
     print "good--file bow_test_time_msstatecv_$letter.html has data <br>";
    if ( $letter === "0" ) {	 
...
$linenavbar = "<a href=\"http://blogs.library.msstate.edu/cdm/scriptoutput/bow_test_time_msstatecv_" . "$letter" . ".html\">$letter</a>\n";  // For non-msstate interface (generic)
...
foreach (glob("bow_test_time_msstatecv_*sorted") as $filename) {
      print "removing $filename <br>";
      unlink($filename) or print("could not unlink $filename ....does not exist <br>");
}  
foreach (glob("bow_test_time_msstatecv_*dedupped") as $filename) {
	  print "removing $filename <br>";
      unlink($filename) or print("could not unlink $filename ....does not exist <br>");
}
foreach (glob("bow_test_time_msstatecv*temp*") as $filename) {
      print "removing $filename <br>";
      unlink($filename) or print("could not unlink $filename ....does not exist <br>");
}
